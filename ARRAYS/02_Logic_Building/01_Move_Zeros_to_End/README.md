...
Traverse the remaining part of the array. Whenever a non-zero element is found, swap it with the element at index `j` and increment `j`. This gradually pushes all zeroes towards the end while preserving the relative order of the non-zero elements.
